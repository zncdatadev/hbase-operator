...
helm-chart-publish: helm helm-chart-package ## Publish helm chart for the operator.
...
# When run `make setup-chainsaw-cluster`, the version of k8s will be used to create the kind cluster,
# and the target kubeconfig file will be named as `./.kubeconfig-$(KIND_K8S_VERSION)`.
# So if you want to use the target cluster, to run `export CHAINSAW_KUBECONFIG=./.kubeconfig-$(KIND_K8S_VERSION)`.
KIND_K8S_VERSION ?= 1.26.15
CHAINSAW_VERSION ?= v0.2.12

KIND_IMAGE ?= kindest/node:v${KIND_K8S_VERSION}
CHAINSAW_KUBECONFIG ?= ./.kubeconfig-$(KIND_K8S_VERSION)
...
setup-chainsaw-cluster: ## Create a kind cluster for chainsaw e2e tests.
	kind create cluster --config $(KIND_CONFIG) --image $(KIND_IMAGE) --name $(KIND_CLUSTER_NAME) --kubeconfig $(CHAINSAW_KUBECONFIG) --wait 120s

.PHONY: delete-chainsaw-cluster
delete-chainsaw-cluster: ## Delete the kind cluster for chainsaw e2e tests.
...
chainsaw-setup: ## DEPRECATED: Use setup-chainsaw-e2e instead
	@echo "Warning: 'chainsaw-setup' is deprecated. Use 'setup-chainsaw-e2e' instead."
	$(MAKE) setup-chainsaw-e2e

.PHONY: setup-chainsaw-e2e
setup-chainsaw-e2e: ## Setup environment for chainsaw e2e tests
	make docker-build
	kind --name $(KIND_CLUSTER_NAME) load docker-image $(IMG)
	KUBECONFIG=$(CHAINSAW_KUBECONFIG) make helm-install-depends
	KUBECONFIG=$(CHAINSAW_KUBECONFIG) make deploy

.PHONY: chainsaw-test
chainsaw-test: ## DEPRECATED: Use chainsaw-e2e instead
	@echo "Warning: 'chainsaw-test' is deprecated. Use 'chainsaw-e2e' instead."
	$(MAKE) chainsaw-e2e

.PHONY: chainsaw-e2e
chainsaw-e2e: chainsaw ## Run chainsaw e2e tests
	KUBECONFIG=$(CHAINSAW_KUBECONFIG) $(CHAINSAW) test --cluster cluster-1=$(CHAINSAW_KUBECONFIG) --test-dir ./test/e2e/

.PHONY: chainsaw-cleanup
chainsaw-cleanup: ## Cleanup chainsaw e2e test environment
	KUBECONFIG=$(CHAINSAW_KUBECONFIG) make helm-uninstall-depends
